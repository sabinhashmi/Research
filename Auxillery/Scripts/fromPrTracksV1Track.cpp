...
namespace {
...
#ifdef GAUDI_FUNCTIONAL_CONSTRUCTOR_USES_TUPLE
    return std::make_tuple( KV{"InputTracksLocation", ""}, KV{ConversionInfo::AncestorLocations[I], ""}... );
#else
    return std::array{KV{"InputTracksLocation", ""}, KV{ConversionInfo::AncestorLocations[I], ""}...};
#endif
  }
  template <typename Base, typename ConversionInfo, size_t... I>
...
#else
    return std::array{KV{"InputTracksLocation1", ""}, KV{"InputTracksLocation2", ""},
                      KV{ConversionInfo::AncestorLocations[I], ""}...};
#endif
  }
  ...
  void ConvertTrack( LHCb::Event::v1::Track* outTr, const PrTrackProxy& inTrack, const StateErrors& errors,
    ...
    if constexpr ( !std::is_same_v<PrTracksType, LHCb::Pr::Fitted::Forward::Tracks> ) {
      const int nHits = inTrack.nHits().cast();
      for ( int iHit = 0; iHit < nHits; iHit++ ) {
        auto lhcbid = inTrack.template get<typename Tag::lhcbIDs>( iHit ).lhcbID().LHCbID();
        outTr->addToLhcbIDs( lhcbid );
      }
    }
    ...
    if constexpr ( std::is_same_v<PrTracksType, LHCb::Pr::Seeding::Tracks> ){
    	outTr->setChi2PerDoF((inTrack.template get<typename Tag::Chi2PerDoF>()).cast());
	outTr->setNDoF((inTrack.template get<typename Tag::nFTHits>()).cast());
    }

//    if constexpr ( std::is_same_v<PrTracksType, LHCb::Pr::Downstream::Tracks> ){
//	auto trackSeeds = *inTrack.getFTAncestors();
//	if (nullptr != trackSeeds){
//		auto trackSeed = trackSeeds[inTrack.get<DownTag::TrackSeed>().cast()];
//    		outTr->setChi2PerDoF((trackSeed.template get<LHCb::Pr::Seeding::Tag::Chi2PerDoF>()).cast );
//		outTr->setNDoF((trackSeed.template get<LHCb::Pr::Seeding::Tag::nFTHits>()).cast );
//	}
//    }
  }
} // namespace
